...
(function ($) {
  ...
  window.ndsYoutubeEmbed = function () {
    ...
    var youtubeEmbedBlock = $(scope).find('.nds-youtube-embed');
    var $wrapper = youtubeEmbedBlock.find('.yt-wrapper');
    var $container = youtubeEmbedBlock.find('.yt-container');
    ...
    function loadYouTubeIframe() {
      ...
      if (!videoId) return;
      var iframe = "<iframe\n                        src=\"https://www.youtube.com/embed/".concat(videoId, "?autoplay=").concat(autoplay ? 1 : 0, "&enablejsapi=1&rel=0&playsinline=1&modestbranding=1&showinfo=0\"\n                        frameborder=\"0\"\n                        allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\"\n                        allowfullscreen\n                        class=\"w-full h-full\"\n                  ></iframe>");
    ...
    var userEvents = ['scroll', 'mousemove', 'touchstart'];
    userEvents.forEach(function (event) {
      return window.addEventListener(event, preloadIframe, {
        once: true
      });
    });
...
// JS FOR FORM 7
var Form7Toggle = /*#__PURE__*/function () {
  function Form7Toggle(form) {
    _classCallCheck(this, Form7Toggle);
    this.form = form;
    this.SELECTEDITEM = 'contact-reason'; // CF7 field name
    if (!form) return;
    this.init();
  }
  return _createClass(Form7Toggle, [{
    key: "init",
    value: function init() {
      var _this = this;
      var wrapper = this.form.querySelector("[data-name=\"".concat(this.SELECTEDITEM, "\"]"));
      if (!wrapper) return;
      var select = wrapper.querySelector('select');
      if (!select) return;

      // Your container (currently id="contact-reason")
      var container = this.form.querySelector("#".concat(this.SELECTEDITEM));
      if (!container) return;

      // The textarea inside it
      var otherField = container.querySelector('[name="other-text-field"], textarea');
      if (!otherField) return;

      // Bind + set initial state
      var toggle = function toggle() {
        return _this.toggleOther(container, otherField, select.value === 'Others');
      };
    ...
    key: "toggleOther",
    value: function toggleOther(container, input, isOther) {
      // Show/hide
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  wpcf7Forms.forEach(function (wpcf7Form) {
    return new Form7Toggle(wpcf7Form);
  });
});
